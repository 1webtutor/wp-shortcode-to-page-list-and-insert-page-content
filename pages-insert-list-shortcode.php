<?php
/*
Plugin Name: Pages Insert & List Shortcode
Author: Ramesh
Plugin URI: http://1webtutor.in/plugin
Description: You can use shortcode for display child pages from the page.
Version: 1.
Author URI: http://1webtutor.in
Domain Path: /languages
Text Domain: pages-insert-shortcode
*/
...
if ( ! function_exists('pagelist_unqprfx_plugin_meta') ) {
	function pagelist_unqprfx_plugin_meta( $links, $file ) { // add 'Plugin page' and 'Donate' links to plugin meta row
		if ( strpos( $file, 'page-list.php' ) !== false ) {
			$links = array_merge( $links, array( '<a href="http://1webtutor.in/plugins/" title="Plugin page">' . __('Page-list') . '</a>' ) );
			}
		return $links;
	}
